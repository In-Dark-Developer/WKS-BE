...
| FR-DT-06 | P0 | 해금은 실 차감과 해금 기록을 한 트랜잭션으로 한다. 이미 해금한 필드는 차감 없이 값만 반환한다. 여러 필드를 한 요청에 묶어 받으며, 잔액이 모자라면 묶음 전체를 해금하지 않는다 (plan §8.5, 2026-09-27) |
...
| FR-DT-10 | P0 | 보낸 사람만 대기 중 요청을 취소할 수 있다. 취소한 쌍은 현재 추천 카드에서 재요청 가능하며, 동시에 유효한 요청은 한 쌍에 하나만 허용한다 (plan §8.6) |
| FR-DT-11 | P0 | 요청 목록에 상대 프로필과 궁합 점수를 제공한다. 받은 목록에서는 발신자의 사진·이름·학과를 무료 공개하고, 보낸 목록에서는 기존 해금 상태를 지킨다. 연락처는 수락 후에만 공개한다 (plan §8.6) |
...
| FR-TH-04 | P0 | 획득: 가입 10(첫 로그인 1회), 출석 5(1일 1회, KST 날짜 기준), 내 궁합지도에 친구 1명 등록 시 +3(공유자, 궁합 1건당 1회), 제휴처 유입(제휴처별 값). 소모: 사진 10 · 이름 7 · 학과 5 · 궁합 까닭 3 · 전체 25 (plan §1.4). 리롤: KST 하루 1회 무료, 이후 5 (2026-09-27, TBD-6 종료) |
