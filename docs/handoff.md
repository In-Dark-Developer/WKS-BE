...
- `ReadingScorer` 에 기둥 가중치(년 0.7·월 1.3·일 1.0·시 0.9, 지지 0.85) 적용 → 십성 합이 소수가 되어 점수 계단이 줄어듦 (점수 종류 결혼 57→66, 자녀 26→43, 연애 39→59). 이어서 카테고리별 선형 보정(중앙값 74, 표준편차 약 14). 1950~2010 고유 팔자 8,225개 기준 분포: 결혼 SS 8%·B 5%, 자녀 SS 15%·B 7%, 연애 SS 7%·B 4%. 상/하 각 약 50/50, 운명 8조합 각 9~21%. 분포 그래프는 `~/Workspace/WKS-BE-analysis/score-distribution.html` (레포 밖)
