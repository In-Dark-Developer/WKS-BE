...
### 2026-09-17 (목) · 차은호 · saju/ '나의 기운' 이 개수와 다른 개념임을 밝히기 (#74) · Claude Code

**한 일**
- "나의 기운"은 일간(태어난 날의 천간) 오행이라 글자 개수와 무관한데, 화면에는 개수만 보여서 "불이 2개뿐인데 왜 불의 기운인 당신?" 으로 읽혔다
- 프롬프트 라벨을 `나의 기운(태어난 날의 기운)` 으로, 시스템 프롬프트에 두 개념의 차이와 "태어난 날에서 온 기운임을 밝힌다" 규칙 추가
- 실호출 3회(불 2개·물 4개, 일간 병): 3회 모두 "태어난 날이 불의 기운이라 …" 로 시작하고 물이 많다는 점을 따로 짚음
...
나의 기운이 최다 기운이 아닌 경우 **44.7%**. 절반 가까이가 이 오해에 노출됐다.

**건드린 파일/패키지**
- `saju/ReadingGenerator.java`(라벨 1줄), `resources/prompts/reading-system.txt`, `ReadingGeneratorTest`, `docs/handoff.md`

**다음 사람이 알아야 할 것**
- 일간은 명리에서 "나" 자체라 개수와 무관한 게 정상이다. 코드를 바꿀 문제가 아니라 설명의 문제였다
- 화면에 일간을 표시하면 근본적으로 해결되지만 응답 필드 추가가 필요하다 (#55 에서 한 번 폐기됨). 지금은 문장으로 해결
