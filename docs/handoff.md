...
### 2026-09-23 (수) · 곽도윤 · main 릴리즈 범위에서 로그인·사진업로드 제외 (release/main-infra-only) · Claude Code

**한 일**
- `dev`→`main` 첫 릴리즈를 준비하다가, `main`이 12일 넘게 방치돼 있었고 심지어 지금 실제로 돌고 있는
  운영(HTTPS·certbot·AWS 연동 포함)과도 전혀 다른 아주 오래된 스냅샷이라는 걸 발견했다. **지금까지
  실제 운영 배포는 `dev`가 해왔다** (구 `deploy.yml`의 `dev` 트리거). 이 사실을 본인에게 보고
- 본인 확인 후, main에는 **인프라(nginx·CI 분리)만 먼저 올리고 로그인·사진업로드(S3, #54)는 나중에
  올리기로 결정**. `dev`(`fd89bf6`)에서 `release/main-infra-only` 브랜치를 새로 따서, `git revert`로
  `fc6a321`(S3, #54)과 `ab20564`(로그인+인프라 묶음 커밋) 두 개를 되돌린 뒤, 인프라 관련 파일만 다시
  얹었다 (`git checkout ab20564 -- <경로>`로 신규 파일 복원 + `docker-compose.prod.yml`/`deploy.yml`
  수동 재패치)
- **되돌린 것**: `auth/`·`common/auth/`·`member/` 패키지 전체, `signup/PhotoUploadService`·`S3Config`
  등 S3 업로드 코드, `build.gradle`의 `nimbus-jose-jwt`·AWS SDK 의존성, `application.yml`의 `app.auth`·
  `app.aws` 섹션, `docker-compose.prod.yml`의 `AWS_*` 환경변수, Flyway `V9~V11`(사진 업로드·member·
  result.member_id 관련 3개, 번호는 리네임 이력이 얽혀 있었음), `ErrorCode`의 `UNAUTHENTICATED`·
  `KAKAO_UNAVAILABLE`
- **유지한 것**: nginx·CI 인프라 전체(`docker-compose.dev.yml`, `nginx/api-dev.*`, `deploy-dev.yml`,
  `ci.yml`, `runbook-dev-server.md`, `deploy.yml`의 `main` 트리거, `docker-compose.prod.yml`의
  `wks-edge` 네트워크·`JAVA_TOOL_OPTIONS`·로그 rotate), signup 관련 이전 기능(V7 프로필 필드 등), 결과
  조회·재사용·닉네임 변경 등 로그인·S3와 무관한 모든 기존 기능
- **검증**: `fc6a321^`(두 기능이 전혀 없던 마지막 커밋)과 `HEAD`를 `ErrorCode.java`·`ResultRepository.java`·
  `Result.java`(#66·#68은 남아있어야 함)·`OpenApiConfig.java`·`common/agents.md`·`build.gradle`·
  `.env.prod.example`에 대해 diff해서, 로그인·S3 흔적이 완전히 없고 무관한 기존 기능(#64 CallBudget 등)은
  그대로인 것 확인. `grep`으로 `auth`·`member`·`PhotoUpload`·`S3Config`·`KAKAO_`·`nimbus-jose`·`awssdk`
  잔존 참조 없음 확인
- **문서는 되돌리지 않았다** — `docs/*`·`AGENTS.md`·`api.md`는 `dev`(`fd89bf6`) 전체 내용을 그대로
  가져왔다. `git revert`가 처음엔 이 문서들까지 되돌리려 해서(핸드오프 로그 전체가 `ab20564` 한 커밋에
  묶여 있었던 탓에 `AGENTS.md`가 0바이트로, `handoff.md`가 200줄 넘게 날아갈 뻔했다), 다시
  `git checkout fd89bf6 -- <문서들>`로 전부 복원했다. 문서가 아직 main에 없는 로그인 얘기를 담고
  있어도 실제 위험은 없다고 판단(문서는 실행되지 않음), 대신 팀의 유일한 공유 기억인 `handoff.md`를
  통째로 날리는 게 훨씬 큰 손실이라고 봤다

**건드린 파일**
- `release/main-infra-only` 브랜치, 로컬 작업. 아직 `main`에 PR 안 올림

**다음 사람이 알아야 할 것**
- 이 브랜치가 `main`에 머지되면 그게 **`main`에 대한 첫 실질적 배포**다 — 로그인·사진업로드 없이
  nginx 분리·CI만 있는 상태로 운영이 재구성된다 (다운타임 예상치·순서는 `docs/runbook-dev-server.md`)
- 로그인·사진업로드는 여전히 `dev`엔 있다. `dev`에서 다시 작업해서 **별도로** `main`에 올릴 시점을
  나중에 정할 것 — 이번 브랜치는 그 작업의 히스토리를 지우지 않았다(`dev`는 안 건드림, `revert`는
  이 브랜치에서만 일어남)
- `main`에 이 브랜치가 들어가면 `application.yml`에 `app.auth`·`app.aws` 섹션이 없다 — 나중에 로그인을
  다시 `main`에 올릴 때는 이 섹션들과 관련 `docker-compose.prod.yml` 환경변수를 다시 추가해야 한다

**막힌 것 / 넘기는 것**
- 곽도윤: `release/main-infra-only` 브랜치 리뷰·PR(base `main`), 로컬 `./gradlew build` 통과 확인 후 머지

**문서 변경**
- 없음 (문서는 `dev`와 동일하게 유지, 위 "한 일" 참고)

**프론트에 알려야 할 것**
- 없음 (API 계약 변경 없음, 이 브랜치가 머지돼도 `main`의 API 표면은 지금 운영과 동일)
